...
int QuasiElastic_ana(const std::string configfilename, std::string filebase="../outfiles/QE_data")
{
  ...
  // gem var
  double nhits[10];
  std::vector<std::string> gemvar = {"nhits"};
  std::vector<void*> gemvar_mem = {&nhits};
  ...
  TString outFile = Form("%s_" + sbsconf.GetSBSconf() + "_sbs%dp_nucleon_%s_model%d_testglobalcuts.root", 
  ...
  //track
  double T_tr_n;	Tout->Branch("ntrack", &T_tr_n,"ntrack/D");
  ...
  //gem
  double T_gem_track_nhits; Tout->Branch("nhits", &T_gem_track_nhits,"nhits/D");
