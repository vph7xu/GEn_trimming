...
int QuasiElastic_ana_withsbsgems_besthcalclus(const std::string configfilename,
                                              std::string filebase="/volatile/halla/sbs/vimukthi/outfiles/He3_newhcalclus/QE_data")
{
  ...
  // hcal primary cluster vars (sbs.hcal.*)
  int Ndata_clus_id,Ndata_clus_e,Ndata_clus_x,Ndata_clus_y,Ndata_clus_atime,Ndata_clus_tdctime,Ndata_clus_nblk;
  ...
  setrootvar::setbranch(C,"Ndata.sbs.hcal.clus","id",&Ndata_clus_id);
  setrootvar::setbranch(C,"Ndata.sbs.hcal.clus","e",&Ndata_clus_e);
  setrootvar::setbranch(C,"Ndata.sbs.hcal.clus","x",&Ndata_clus_x);
  setrootvar::setbranch(C,"Ndata.sbs.hcal.clus","y",&Ndata_clus_y);
  setrootvar::setbranch(C,"Ndata.sbs.hcal.clus","atimeblk",&Ndata_clus_atime);
  setrootvar::setbranch(C,"Ndata.sbs.hcal.clus","tdctimeblk",&Ndata_clus_tdctime);
  setrootvar::setbranch(C,"Ndata.sbs.hcal.clus","nblk",&Ndata_clus_nblk);

  // hcal ALL cluster vars (sbs.hcal.clus.*)
  ...
  // hcal clus mem (unused; keep as-is)
  ...
  // track var
  double ntrack, chi2[maxNtr], p[maxNtr],px[maxNtr],py[maxNtr],pz[maxNtr],xTr[maxNtr],yTr[maxNtr],thTr[maxNtr],phTr[maxNtr];
  ...
  double x[maxNtr],y[maxNtr],th[maxNtr],ph[maxNtr];
  std::vector<std::string> trvar = {"n","chi2","p","px","py","pz","vx","vy","vz","tg_x","tg_y","tg_th","tg_ph","r_x","r_y","r_th","r_ph","x","y","th","ph"};
  std::vector<void*> trvar_mem = {&ntrack,chi2,p,px,py,pz,vx,vy,vz,xtgt,ytgt,thtgt,phtgt,xfp,yfp,thfp,phfp,x,y,th,ph};
  ...
  int hcal_best_idx;    Tout->Branch("hcal_best_idx", &hcal_best_idx, "hcal_best_idx/I");
  bool hcal_best_pass;  Tout->Branch("hcal_best_pass", &hcal_best_pass, "hcal_best_pass/O");
  ...
  double T_ntrack_chi2;     Tout->Branch("ntrack_chi2", &T_ntrack_chi2, "ntrack_chi2/D");
  double T_ntrack_hits;     Tout->Branch("ntrack_hits", &T_ntrack_hits, "ntrack_hits/D");
  double T_track_x;         Tout->Branch("track_x", &T_track_x,"track_x/D");
  double T_track_y;         Tout->Branch("track_y", &T_track_y,"track_y/D");
  double T_track_xp;        Tout->Branch("track_xp", &T_track_xp,"track_xp/D");
  double T_track_yp;        Tout->Branch("track_yp", &T_track_yp,"track_yp/D");
  ...
  double T_x;               Tout->Branch("x", &T_x, "x/D");
  double T_y;               Tout->Branch("y", &T_y, "y/D");
  double T_th;              Tout->Branch("th", &T_th, "th/D");
  double T_ph;              Tout->Branch("ph", &T_ph, "ph/D");
  double T_xBB;             Tout->Branch("xBB", &T_xBB, "xBB/D");
  double T_yBB;             Tout->Branch("yBB", &T_yBB, "yBB/D");
  ...
  double T_ntrack_chi2_sbs;     Tout->Branch("ntrack_chi2_sbs", &T_ntrack_chi2_sbs, "ntrack_chi2_sbs/D");
  ...
  // HCAL all clus
  int T_Ndata_clus_id;                  Tout->Branch("Ndata_clus_id",&T_Ndata_clus_id,"Ndata_clus_id/I");
  //int T_Ndata_clus_e;                  Tout->Branch("Ndata_clus_e",&T_Ndata_clus_e,"Ndata_clus_e/I");
  //int T_Ndata_clus_x;                  Tout->Branch("Ndata_clus_x",&T_Ndata_clus_x,"Ndata_clus_x/I");
  //int T_Ndata_clus_y;                  Tout->Branch("Ndata_clus_y",&T_Ndata_clus_y,"Ndata_clus_y/I");
  //int T_Ndata_clus_atime;                  Tout->Branch("Ndata_clus_atime",&T_Ndata_clus_atime,"Ndata_clus_atime/I");
  //int T_Ndata_clus_tdctime;                  Tout->Branch("Ndata_clus_tdctime",&T_Ndata_clus_tdctime,"Ndata_clus_tdctime/I");
  //int T_Ndata_clus_nblk;                  Tout->Branch("Ndata_clus_nblk",&T_Ndata_clus_nblk,"Ndata_clus_nblk/I");
  double T_hcal_clus_E[maxhcal];        Tout->Branch("hcal_clus_e", T_hcal_clus_E, "hcal_clus_e[Ndata_clus_id]/D");
  double T_hcal_clus_x[maxhcal];        Tout->Branch("hcal_clus_x", T_hcal_clus_x, "hcal_clus_x[Ndata_clus_id]/D");
  double T_hcal_clus_y[maxhcal];        Tout->Branch("hcal_clus_y", T_hcal_clus_y, "hcal_clus_y[Ndata_clus_id]/D");
  double T_hcal_clus_atime[maxhcal];    Tout->Branch("hcal_clus_atime", T_hcal_clus_atime, "hcal_clus_atime[Ndata_clus_id]/D");
  double T_hcal_clus_tdctime[maxhcal];  Tout->Branch("hcal_clus_tdctime", T_hcal_clus_tdctime, "hcal_clus_tdctime[Ndata_clus_id]/D");
  double T_hcal_clus_id[maxhcal];       Tout->Branch("hcal_clus_id", T_hcal_clus_id, "hcal_clus_id[Ndata_clus_id]/D");
  double T_hcal_clus_nblk[maxhcal];     Tout->Branch("hcal_clus_nblk", T_hcal_clus_nblk, "hcal_clus_nblk[Ndata_clus_id]/D");
  ...
  // binary search helper on epics_ev
  auto up = [&](double ev){
    return std::upper_bound(epics_evnum.begin(), epics_evnum.end(), ev) - epics_evnum.begin();
  };
  ...
  while (C->GetEntry(nevent++)) {
    ...
    if (nevent == 1 || currenttreenum != treenum) {
      ...
      // Determine IHWP state for the run
    ...
    // Timing Information: BB time is known now; HCAL time set after best cluster selection
    ...
    // ------------------------------------------------------------------
    // BEST HCAL CLUSTER (use sbs.hcal.clus.* arrays):
    //   1) Sort cluster arrays by descending energy (in-place)
    //   2) Pick first cluster among top-5 that passes coincidence-time cut
    // ------------------------------------------------------------------
    int nclus_sel = std::min((int)Ndata_clus_id, maxhcal);

    HCalClusterSelector::SortByEnergy(
      nclus_sel,
      clusEHCAL,
      clusatimeHCAL,
      clusxHCAL,
      clusyHCAL,
      maxhcal,            // <-- FIXED: maxclus int goes here
      clustdctimeHCAL,
      blkid,
      nblk
    );

    auto best = HCalClusterSelector::SelectBestByCoincidence(
      nclus_sel,
      clusatimeHCAL,
      atimeSH,
      coin_time_cut[0],
    ...
    int ihcal = best.best_idx;
    hcal_best_idx = ihcal;
    hcal_best_pass = best.pass;

    // Convenience aliases for the selected cluster
    double eHCAL_best = clusEHCAL[ihcal];
    double xHCAL_best = clusxHCAL[ihcal];
    double yHCAL_best = clusyHCAL[ihcal];
    double atimeHCAL_best = clusatimeHCAL[ihcal];
    double tdctimeHCAL_best = clustdctimeHCAL[ihcal];

    // NOW it's valid to fill HCAL time:
    T_hcal_time = atimeHCAL_best;

    // Best-cluster block row/col (approx) from blkid assuming 12 columns:
    int blk_best = (int)blkid[ihcal];
    int cblk_best = (blk_best >= 0) ? (blk_best % 12) : -1;
    int rblk_best = (blk_best >= 0) ? (blk_best / 12) : -1;

    double coin_time = atimeHCAL_best - atimeSH;
    T_coin_time = coin_time;
    h_coin_time->Fill(coin_time);

    coinCut = coin_time > coin_time_cut[0] - Nsigma_coin_time*coin_time_cut[1] &&
              coin_time < coin_time_cut[0] + Nsigma_coin_time*coin_time_cut[1];
    ...
    // ---- Everything below here is unchanged logic from your pasted macro ----
    // (Your original code continues…)
    //
    // NOTE: I’m keeping your downstream usage of:
    //   eHCAL_best, xHCAL_best, yHCAL_best, atimeHCAL_best, etc.
    // which now compile correctly and are consistent with "best cluster".

    size_t j   = up(evnum_T);
    size_t idx = (j==0) ? 0 : j-1;
    if(idx >= epics_HALLA_p.size()) idx = epics_HALLA_p.size()-1;

    double pMeV = epics_HALLA_p[idx];
    double ebeam = pMeV/1000.0;

    double ebeam_corr = ebeam;
    double precon = p[0];

    TVector3 vertex(0, 0, vz[0]);
    TLorentzVector Pe(0,0,ebeam_corr,ebeam_corr);
    TLorentzVector Peprime(px[0] * (precon/p[0]),
                           py[0] * (precon/p[0]),
                           pz[0] * (precon/p[0]),
                           precon);
    TLorentzVector PN;
    ...
    T_ntrack = ntrack;
    T_ntrack_chi2 = chi2[0];
    T_ntrack_hits = bb_gem_nhits[0];
    T_track_x = bb_gem_track_x[0];
    T_track_y = bb_gem_track_y[0];
    T_track_xp = bb_gem_track_xp[0];
    T_track_yp = bb_gem_track_yp[0];
    T_vz = vz[0];
    T_vx = vx[0];
    T_vy = vy[0];
    T_xtgt = xtgt[0];
    ...
    T_th = th[0];
    T_ph = ph[0];
    T_xBB = xfp[0]-0.9*thfp[0];
    T_xBB = xfp[0]-0.9*thfp[0];
    T_yBB = yfp[0]-0.9*phfp[0];
    T_trP = p[0];
    T_trPx = px[0];
    T_trPy = py[0];
    T_trPz = pz[0];
    ...
    double theta_pq = kine::theta_pq(vertex, HCAL_origin, pNhat, xHCAL_best, yHCAL_best);
    kine::GetxyHCALexpect(vertex, pNhat, HCAL_origin, HCAL_axes, xyHCAL_exp);
    double dx = xHCAL_best - xyHCAL_exp[0];
    double dy = yHCAL_best - xyHCAL_exp[1];
    ...
    bool AR_cut = cut::inHCAL_activeA(xHCAL_best, yHCAL_best, hcal_active_area);
    ...
    if (WCut && coinCut) {
      h_dxHCAL->Fill(dx);
      h_dyHCAL->Fill(dy);
      h2_rcHCAL->Fill(cblk_best, rblk_best);
