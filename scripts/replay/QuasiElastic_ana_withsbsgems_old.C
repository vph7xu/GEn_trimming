...
int QuasiElastic_ana(const std::string configfilename, std::string filebase="../outfiles/QE_data")
{
  ...
  std::vector<std::string> sbstrvar ={"n","p","px","py","pz","vx","vy","vz","tg_x","tg_y","tg_th","tg_ph","r_x","r_y","r_th","r_ph"};
  std::vector<void*> sbstrvar_mem = {&ntrack_sbs,&p_sbs,&px_sbs,&py_sbs,&pz_sbs,&vx_sbs,&vy_sbs,&vz_sbs,&xtgt_sbs,&ytgt_sbs,&thtgt_sbs,&phtgt_sbs,&xfp_sbs,&yfp_sbs,&thfp_sbs,&phfp_sbs};
  ...
  TString outFile = Form("%s_" + sbsconf.GetSBSconf() + "_sbs%dp_nucleon_%s_model%d_with_sbs.root", 
  ...
  //these are just me looking at SBS tracking data
  TH1F *h_dxHCAL_sbs_tr_n =  new TH1F("h_dxHCAL_sbs_tr_n","; x_{HCAL} - x_{exp} (m);",int(hdx_lim[0]),hdx_lim[1],hdx_lim[2]);  
  TH1F *h_dxHCAL_sbs_tr_p =  new TH1F("h_dxHCAL_sbs_tr_p","; x_{HCAL} - x_{exp} (m);",int(hdx_lim[0]),hdx_lim[1],hdx_lim[2]);
  ...
  while (C->GetEntry(nevent++)) {
	...
	//}
	if (ntrack_sbs==0) h_dxHCAL_sbs_tr_n->Fill(dx);
        else h_dxHCAL_sbs_tr_p->Fill(dx);
  ...
  c2->cd(1);
  h_dxHCAL_sbs_tr_p->Draw();
  h_dxHCAL_sbs_tr_n->SetLineColor(kRed);
  h_dxHCAL_sbs_tr_n->Draw("same");

  TLegend *legend = new TLegend(0.7,0.7,0.9,0.9);
  legend->AddEntry(h_dxHCAL_sbs_tr_p,"sbs track");
  legend->AddEntry(h_dxHCAL_sbs_tr_n,"no_sbs_track");
  legend->Draw();
